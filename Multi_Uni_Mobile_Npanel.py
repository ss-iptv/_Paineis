...
SIMULTANEOUS_SITES = 5  # 2 sites simultâneos
...
def get_first_combo_file():
    try:
        combo_dir = "/sdcard/NPANEL//multi_combo"
...
def read_sites():
    try:
        with open("/sdcard/NPANEL//site.txt", "r") as f:
...
def save_data(nhost, user, password, credits, email, registration_date, twofa, whatsapp, telegram):
    ...
    with open("/sdcard/NPANEL//hits/Sr.Hell@COMBO(U&P).txt", "a") as f:
